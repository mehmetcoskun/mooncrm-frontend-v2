...
export function DataTableBulkActions<TData>({
  ...
  useEffect(() => {
    if (selectedCount > 0) {
      const message = `${selectedCount} ${entityName}${selectedCount > 1 ? 's' : ''} seçildi. Toplu eylemler araç çubuğu kullanılabilir.`;
  ...
  return (
    <>
      ...
      <div
        ref={toolbarRef}
        role="toolbar"
        aria-label={`Toplu eylemler için ${selectedCount} seçildi ${entityName}${selectedCount > 1 ? 's' : ''}`}
        ...
        >
          ...
          >
            <Badge
              ...
              aria-label={`${selectedCount} seçildi`}
            >
              {selectedCount}
            </Badge>{' '}
