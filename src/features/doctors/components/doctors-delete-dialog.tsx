...
import { destroyDoctor } from '@/services/doctor-service';
...
export function DoctorsDeleteDialog({
  ...
  const handleDelete = async () => {
    try {
      setIsLoading(true);
      await destroyDoctor(currentRow.id);

      onOpenChange(false);
      toast.success('Doktor silindi', {
        description: `${currentRow.name} adlı doktor başarıyla silindi.`,
      });
    ...
    <ConfirmDialog
      ...
      desc={
        <div className="space-y-4">
          <p className="mb-2">
            <span className="font-bold">{currentRow.name}</span> adlı doktoru silmek
            istediğinizden emin misiniz?
            <br />
            Bu işlem, <span className="font-bold">{currentRow.name}</span> adlı
            doktori sistemden kalıcı olarak kaldıracaktır. Bu işlem geri
            alınamaz.
          </p>
